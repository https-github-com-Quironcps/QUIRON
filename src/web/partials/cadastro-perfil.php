<!DOCTYPE html>
<html lang="pt-br" class="light">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">

    <link rel="stylesheet" href="../styles/theme.css">
    <link rel="stylesheet" href="styles/styles_g/style.css">

    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.5.0/font/bootstrap-icons.css">
    <script src="scripts/ImagenError.js"></script>
    <script src="scripts/FechaDiv.js"></script>

    <link rel="icon" href="../web/images/logos/arco-dark-2.png">

    <title>QUIRON - CONFIRMAÇÃO</title>
</head>
<body>
    <div id="fecharDiv" class="div-todo">

        <div style="height: 450px;" class="principal">
            <center>
                <div class="conteudo6">
                    
                        <i id="star" class="bi bi-stars"></i><br>
                    
                        <i id="star2" class="bi bi-stars"></i>
              
                    <div class="confirm">
                        <i id="icon" class="bi bi-check-lg"></i>
                    </div><br><br>

                    <h3 class="mensagem">Cadastrado! Agora, faça seu perfil de Visualização</h3>

                    <input style="height: 45px; width:110px;" type="submit" onclick="sairTela()" value="Continuar" class="btn-enviar">
                </div>
            </center>
        </div>
        </div>
</body>
</html>